...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    ...
    cert.data = (unsigned char *)kCertificateDER;
    cert.size = sizeof(kCertificateDER);
    key.data = (unsigned char *)kRSAPrivateKeyDER;
    key.size = sizeof(kRSAPrivateKeyDER);
    res = gnutls_certificate_set_x509_key_mem(
        xcred, &cert, &key, GNUTLS_X509_FMT_DER);
    assert(res >= 0);
