...
#ifndef GNUTLS_LIB_DLWRAP_TSS2_MU_H_
#define GNUTLS_LIB_DLWRAP_TSS2_MU_H_
...
#define GNUTLS_TSS2_MU_FUNC(name) gnutls_tss2_mu_func_##name

#else

#define GNUTLS_TSS2_MU_FUNC(name) name
 ...
 */
int gnutls_tss2_mu_ensure_library (const char *soname, int flags);
 ...
 * multi-threaded programs, protect it with a locking mechanism.
 */
void gnutls_tss2_mu_unload_library (void);
 ...
 * multi-threaded programs, protect it with a locking mechanism.
 */
unsigned gnutls_tss2_mu_is_usable (void);

#endif /* GNUTLS_LIB_DLWRAP_TSS2_MU_H_ */
